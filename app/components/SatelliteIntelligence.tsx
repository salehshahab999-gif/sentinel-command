...
export default function SatelliteIntelligence() {
  ...
  useEffect(() => {
    ...
    setRuntimeStatus("POWERING GLOBE / KNOWN-GOOD BASELINE / LIVE OFF");
...
>>>>>>> 6bb5105c40394eb5ee86b3497d6ea85f657e7116
    ...
    viewer.scene.requestRender();
    setRuntimeStatus("CESIUM 3D READY / FREE MAP CHAIN / LIVE OFF");
  ...
  useEffect(() => {
    const handleMapFocus = (event: Event) => {
      ...
      viewer.camera.flyTo({
        destination: Cesium.Cartesian3.fromDegrees(result.longitude, result.latitude, 1_200_000),
        duration: 0.9,
      });
    };
      ...
      <header className="absolute inset-x-0 top-0 z-20 flex items-start justify-between gap-3 p-4 md:p-6">
        <div className="rounded-2xl border border-cyan-950/80 bg-black/70 px-4 py-3 backdrop-blur-xl">
          ...
          <p className="mt-1 text-[9px] tracking-[.18em] text-slate-500">
            CESIUMJS {CESIUM_VERSION} • FREE MAP CHAIN • LIVE SATELLITE FEEDS OFF
          </p>
        </div>
      ...
      <aside className="absolute left-4 top-28 z-20 w-[290px] max-w-[calc(100vw-2rem)] space-y-3 md:left-6">
        <section className="rounded-2xl border border-cyan-950/80 bg-black/75 p-3 backdrop-blur-xl">
          <div className="mb-2 flex items-center justify-between">
            <span className="text-[9px] font-bold tracking-[.24em] text-slate-400">SATELLITE FILTERS</span>
            <span className="text-[8px] text-slate-600">{filteredSatellites.length} OBJECTS</span>
          </div>
          <input
            value={search}
          ...
          <div className="grid grid-cols-4 gap-1">
            {providerFilters.map((item) => (
              <button
                ...
                className={`rounded-md border px-2 py-1.5 text-[8px] font-bold tracking-[.08em] ${provider === item.id ? "border-cyan-700 bg-cyan-950/40 text-cyan-300" : "border-slate-900 text-slate-600"}`}
              >
                {item.label}
              </button>
            ))}
...
=======
        <section className="rounded-2xl border border-cyan-950/80 bg-black/75 p-3 backdrop-blur-xl">
          <div className="mb-2 flex items-center justify-between">
            <span className="text-[9px] font-bold tracking-[.24em] text-slate-400">LAYER BUS</span>
            <span className="text-[8px] text-emerald-500">WIRED</span>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {SATELLITE_LAYERS.map((layer) => (
              <button
                ...
                className={`rounded-md border px-2 py-1.5 text-[8px] font-medium ${enabledLayers[layer.id] ? `${layerAccent[layer.id]} bg-white/[.04]` : "border-slate-900 text-slate-700"}`}
              >
                {layer.label}
              </button>
            ))}
          ...
          <p className="mt-2 text-[8px] leading-relaxed text-slate-600">
            Globe rendering follows the known-good Cesium baseline. Map tiles use the Sentinel free provider chain.
          </p>
        </section>
>>>>>>> 6bb5105c40394eb5ee86b3497d6ea85f657e7116
      </aside>
      ...
      <section className="absolute right-4 top-28 z-20 hidden w-[260px] md:block">
        <div className="rounded-2xl border border-cyan-950/80 bg-black/75 p-4 backdrop-blur-xl">
          <p className="text-[8px] font-bold tracking-[.28em] text-cyan-500">SATELLITE PIPELINE</p>
          <div className="mt-3 space-y-2 text-[9px]">
            <div className="flex justify-between"><span className="text-slate-600">CATALOG</span><span className="text-emerald-400">READY</span></div>
            <div className="flex justify-between"><span className="text-slate-600">CESIUM GLOBE</span><span className={cesiumReady ? "text-emerald-400" : "text-amber-400"}>{cesiumReady ? "READY" : "BOOTING"}</span></div>
            <div className="flex justify-between"><span className="text-slate-600">LIVE COLLECTORS</span><span className="text-slate-500">OFF</span></div>
            <div className="flex justify-between"><span className="text-slate-600">MAP PROVIDERS</span><span className="text-cyan-400">CARTO / ESRI / OSM</span></div>
          </div>
        </div>
      </section>

<<<<<<< HEAD
     <footer className="absolute inset-x-3 bottom-3 z-20 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 rounded-xl border border-cyan-800/70 bg-black/90 px-5 py-3 text-[10px] font-bold tracking-[0.14em] shadow-[0_0_24px_rgba(0,180,255,0.10)] backdrop-blur-none md:inset-x-5">
  
  <span className="text-emerald-400">● CESIUM 3D READY</span>
  <span className="text-cyan-300">● MAP LOCAL-FIRST</span>
  <span className="text-cyan-300">● UNIVERSAL FILTER</span>
  <span className="text-violet-400">● ORBIT ENGINE OFF</span>
  <span className="text-amber-400">● LIVE POWER OFF</span>
  <span className="text-slate-300">● © OpenStreetMap contributors</span>
  </footer>
=======
      {selected && (
        <section className="absolute bottom-20 right-4 z-30 w-[285px] max-w-[calc(100vw-2rem)] rounded-2xl border border-cyan-900/80 bg-black/85 p-4 backdrop-blur-xl md:right-6">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[8px] font-bold tracking-[.25em] text-cyan-500">SELECTED SATELLITE</p>
              <h2 className="mt-1 text-lg font-semibold text-cyan-100">{selected.name}</h2>
              <p className="text-[9px] text-slate-600">{selected.id} • NORAD {selected.noradId ?? "PENDING"}</p>
            </div>
      ...
      <footer className="absolute inset-x-0 bottom-4 z-20 px-4">
        <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-center gap-x-6 gap-y-2 rounded-2xl border border-cyan-950/80 bg-black/75 px-4 py-3 text-[8px] tracking-[.16em] backdrop-blur-xl">
          ...
          <span className="text-cyan-400">● FREE MAP CHAIN</span>
          <span className="text-violet-400">● FILTER BUS WIRED</span>
          <span className="text-amber-400">● POWER ON</span>
