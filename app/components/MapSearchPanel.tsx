...
export default function MapSearchPanel() {
  ...
  const runSearch = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/map/search?q=${encodeURIComponent(value)}`, { cache: "no-store" });
  ...
  return (
    <section className="absolute left-3 top-[calc(6rem+19rem)] z-30 w-[250px] rounded-xl border border-cyan-950/80 bg-black/78 p-3 shadow-2xl backdrop-blur-xl md:left-5 md:w-[285px]">
      ...
      <div className="mt-2 flex gap-1.5">
        ...
        <button
          ...
          disabled={loading}
          className="rounded-lg border border-cyan-800 bg-cyan-950/40 px-3 py-2 text-[9px] font-bold text-cyan-300 hover:bg-cyan-900/40 disabled:opacity-50"
        >
          🔎
        </button>
      </div>
        ...
        <div className="mt-2 max-h-52 space-y-1 overflow-auto pr-1">
          {results.map((result) => (
            <button
              ...
              className="flex w-full items-start justify-between gap-2 rounded-lg border border-slate-900 bg-white/[0.015] px-2 py-2 text-left hover:border-cyan-900"
            >
      ...
      {results.length > 0 && (
        <button
          type="button"
          onClick={() => window.open(results[0].openMapsUrl, "_blank", "noopener,noreferrer")}
          className="mt-2 w-full rounded-md border border-slate-800 px-2 py-1.5 text-[8px] font-bold tracking-[0.12em] text-slate-500 hover:border-slate-700 hover:text-slate-200"
        >
